...
import { BookmarkedItem, HistoryItem } from '../types';
import BookmarkListIcon from './icons/BookmarkListIcon';

interface BookmarksSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  bookmarks: BookmarkedItem[];
  onLoadBookmark: (item: HistoryItem) => void;
}

const BookmarksSidebar: React.FC<BookmarksSidebarProps> = ({ isOpen, onClose, bookmarks, onLoadBookmark }) => {
  return (
    <>
      <div 
        ...
        <div className="flex flex-col h-full">
          <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
            <div className="flex items-center gap-3">
                <BookmarkListIcon className="w-6 h-6 text-sky-500 dark:text-sky-400" />
                <h2 className="text-xl font-semibold font-heading text-slate-800 dark:text-slate-100">Your Bookmarks</h2>
            </div>
          ...
          <div className="overflow-y-auto flex-grow p-4">
            {bookmarks.length === 0 ? (
              <div className="text-center text-slate-500 dark:text-slate-400 h-full flex flex-col items-center justify-center">
                 ...
                 <p className="text-sm">Click the bookmark icon on a topic to save it here!</p>
              </div>
            ) : (
              <ul className="space-y-3">
                {bookmarks.map(({ node, parentHistoryItem }) => (
                  <li key={`${parentHistoryItem.id}-${node.id}`}>
                    <button 
                        onClick={() => onLoadBookmark(parentHistoryItem)}
                    ...
                    >
                      <p className="font-semibold text-slate-800 dark:text-slate-100 truncate">{node.topic}</p>
                      <div className="flex items-center justify-between mt-1 text-sm text-slate-500 dark:text-slate-400">
                          <span>From: {parentHistoryItem.topic}</span>
                          <span className="capitalize px-2 py-0.5 bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-300 rounded-full text-xs font-medium">{parentHistoryItem.level}</span>
                      </div>
                    </button>
                  </li>
                ))}
